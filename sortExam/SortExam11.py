...
# 特殊人的时间
def getSpecialTime(session, firstSession, firstMonth, firstDay):
    global s_begin_year
    global s_begin_month
    global s_begin_day
    global s_dayNum
    ...
    year, month, day = calculate_new_date(s_begin_year, firstMonth, firstDay, (session - firstSession) / s_dayNum)
...
class Person:
    ...
    # 设置特殊session，后续阶段的
    def setSpecialSession(self, session, firstSession, firstMonth, firstDay):
        for row in self.rows:
            time, tag = getSpecialTime(session, firstSession, firstMonth, firstDay)
            row.addSpecialValues(session, time, tag, "special")
            session += 1
...
class ExcelRow:
    ...
    # 加：场次，时间，时间编号, 特殊标记
    def addSpecialValues(self, session, time, tag, special):
        self.values.append(session)
        self.values.append(time)
        self.timeIndex = len(self.values) - 1
        self.values.append(tag)
        self.values.append(special)
...
def readSpecailXlsx():
    ...
    _idIndex = 0
    ids = []
    workbook = openpyxl.load_workbook(s_extraFile)
    if len(workbook.sheetnames) == 0:
        print(u"excel 文件内容为空！！")
        exit()
    elif len(workbook.sheetnames) > 1:
    ...
    for _row in sheet.iter_rows():
        if row_index > 0:
            row = ExcelRow(_row)
            id = row.values[_idIndex]
            if id not in ids:
                ids.append(id)
        row_index += 1

    print(u"特殊人数：{}".format(len(ids)))
    return ids

# 分离特殊人群
def separatePerson(personArr, specialIds):
    result = []
    resultSpecial = []
    for p in personArr:
        if p.getKey() in specialIds:
...
def generateResult(headRow, personArr, specailPersonArr, resultFile):
    ...
    for person in personArr:
        ...
        _endSession = person.setSession(session)
        personIndex += 1
    ...
    # 获取从哪场开始排:前面的最后一场 + 1
    nowSession = _endSession + 1
    for person in specailPersonArr:
        _personKey = person.getKey()
        # addSessionInfo(_sessionInfo, nowSession, person)
        # _sortKeys.append(_personKey)
        # 7 月 5 日开始安排
        person.setSpecialSession(nowSession, nowSession, 7, 5)
        personIndex += 1
        for index, row in enumerate(person.getRows()):
            for _index, value in enumerate(row.values):
                # row, column 从 1 开始
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        exit()
    if len(sys.argv) < 3:
        print(u"请输入要特殊处理的文件名!")
        exit()
    s_originalFile = sys.argv[1]
    s_extraFile = sys.argv[2]
    if not os.path.isfile(s_originalFile) or not os.path.isfile(s_extraFile):
        print(u"输入的文件不存在！")
        exit()
    ...
    # 分离特殊角色
    personArr, specailPersonArr= separatePerson(personArr, _specialIds)
